...
class sms_sts(protocol_packet_handler):
    ...
    def PWMMode(self, scs_id):
        return self.write1ByteTxRx(scs_id, SMS_STS_MODE, 2)
    
    def WriteTorque(self, scs_id, torque_limit, max_torque=500):
        self.writeTxRx(scs_id, 48, 2, [self.scs_lobyte(max_torque), self.scs_hibyte(max_torque)])
        self.writeTxRx(scs_id, 16, 2, [self.scs_lobyte(torque_limit), self.scs_hibyte(torque_limit)])

    def WriteTime(self, scs_id, time):
        self.writeTxRx(scs_id, SMS_STS_GOAL_TIME_L, 2, [self.scs_lobyte(time), self.scs_hibyte(time)])
